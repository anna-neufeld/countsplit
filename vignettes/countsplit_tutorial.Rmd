...
We now compare the slopes and intercepts that we estimate via count splitting to those that we would get in the ideal setting where we were able to observe the true clusters. If we observed the true clusters there would be no double dipping concern and we would be able to use the entire dataset $X$ for inference. 
...
This plot shows what we get. In general, the ideal slopes and the count splitting slopes match up quite well. The count splitting intercepts tend to fall on the line $\log(0.5) + \text{ideal intercept}$. There are a few exceptions where the intercepts are really far from this line. Investigating further, these outliers are cases where XXXXX. 
```{r}
library(ggplot2)
library(patchwork)
...
Overall, when there are true clusters in the data, count splitting agrees pretty well with the ideal method. 