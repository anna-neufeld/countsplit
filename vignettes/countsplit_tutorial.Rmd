...
The plot below shows that the intercepts tend to fall on the line $\text{intercept from } X^\mathrm{test} = \log(0.5) + \text{intercept from X}$. The slopes tend to fall on the line $\text{slope from } X^\mathrm{test} = \text{slope} from X}$. 
```{r}
library(ggplot2)
library(patchwork)
