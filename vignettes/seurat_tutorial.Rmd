...
The purpose of this tutorial is to reproduce the analysis from the Seurat [clustering](https://satijalab.org/seurat/articles/pbmc3k_tutorial.html) tutorial while using `countsplit` to avoid the double dipping that occurs in this tutorial. Briefly, the Seurat tutorial performs clustering and differential expression on the same dataset. As shown in our [introductory tutorial](https://anna-neufeld.github.io/countsplit/articles/countsplit_tutorial.html), this can lead to inflated Type 1 error rates during differential expression testing. For more information on the issues associated with double dipping, please see [add link to prepint](XXXXX). 

Throughout this tutorial, we refer to the June, 2022 version of the Seurat tutorial. In the event that the Seurat tutorial at the link above gets modified, we have also reproduced the relevant analyses below for reference, and can provide the arxived version of the Seurat tutorial upon request. 

<!-- Anna note to self- I git pulled from Seurat and so I have their June 2022 vignette html and should be able to reproduce. Knitting issues tho so need to figure those out.  -->
...
Catch up on full data preprocessing. Note that dimensions won't match but we'll worry about that when we get to it. 
```{r}
...
Ok the adjusted rand index is pretty high! Maybe not QUITE as high as I previously expected. 
...
We can see from this table that their cluster 0 is the same as our cluster 0 and their cluster 2 is the same as our cluster 2 (with some mistakes). their cluster 1 is our cluster 1 (with very few mistakes). 
Their cluster 3 is our cluster 4 and vis versa. Their cluster 5 is our cluster 5 with very few miustakes. They have an extra two clusters!!! But overall we really aren't losing much. 
```{r}
...
We identify ``LTB``, ``IL32``, ``CD3D``, ``IL7R``, and ``LDHB`` as the top markers of cluster 2, which is exactly the same as in the Seurat tutorial (although the order is slightly different and the p-values are slightly different). We reproduce the Seurat findings ``by hand" below; there are small differences in the p-values that may be due to numerical error. 

```{r}
cluster2.full = Idents(pbmc)==2
Xsubset <- pbmc.counts[rownames(pbmc),colnames(pbmc)]
sf.full <- colSums(Xsubset)
Xsubset_norm <- t(apply(Xsubset, 1, function(u) u/sf))
Xsubset_lognorm <- log(Xsubset_norm +1)
pvals2.full <- apply(Xsubset_lognorm, 1, function(u) wilcox.test(u~cluster2.full)$p.value)
head(sort(pvals2))
head(sort(pvals2.full))
```
...
We can also verify that the selected genes match up to what we would have found on the full dataset.
