...
We can see that we obtain very similar principal components on the training set to those obtained on the full data. On both datasets, PC_1 is dominated by the gene `MALAT1` and genes like genes like `CST3, TYROBF` are of lesser importance. (The fact that `MALAT1` has a positive loading in `pbmc.train` and a negative loading in `pbmc` is simply due to a sign flip of the first principal component.) The second principal component is dominated by `CD79A` and `HLA-D0A1` and `HLA-DOB1`, with `NKG7` having lesser importance. (The sign flip is once again unimportant.)
...
We want to know how similar these clusters are to the ones computed on the full data. Looking at two UMAP plots could be potentially misleading, as the UMAP dimensions on the full dataset are \emph{different} than those on the training set. Further complicating our ability to compare the two clusterings, the training set has slightly fewer cells in it due to the preprocessing pipeline. 
