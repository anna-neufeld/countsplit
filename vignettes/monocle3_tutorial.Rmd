...
vignette: >
  %\VignetteIndexEntry{Tutorial: countsplitting and Monocle3}
...
# Preprocess and estimate pseudotime

We now follow the preprocessing and pseudotime steps from the Monocle3 tutorial to come up with pseudotime estiamtes. Everything in this section uses the training set only. 
...
We now have estimated pseudotime, and we can color our cells by pseudotime in our plot.
...
First, we carry out differential expression testing "by hand". We fit a quasipoisson GLM of each test set gene onto the pseudotime computed from the training set. For computational efficiency, we only do this for the first 500 genes. We include the size factors estimated on the training as offsets in the regression.
...
We can also select specific genes and plot their expression across pseudotime. As in the Monocle3 vignette, we see that all three of these genes get "activated" midway through the trajectory, but `dac-1` is activated slightly before the otheres. 
...
As in our [Seurat tuorial](https://anna-neufeld.github.io/countsplit/articles/seurat_tutorial.html), we see that a count splitting analysis yields similar conclusions to analyses that use the naive method (i.e. that use the same data to compute pseudotime and study differential expression). However, by avoiding double dipping, the analysis that uses count splitting more reliably answers the question of ``which genes are differentially expressed"? Thus, in this vignette, we have reproduced the analysis from the Monocle3 vignette, but have improved the reliability of this analysis. 
