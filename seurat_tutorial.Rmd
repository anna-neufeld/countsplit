...
vignette: >
  %\VignetteIndexEntry{"Tutorial: counts plitting with seurat"}
...
We use the same dataset as in the Seurat [clutering](https://satijalab.org/seurat/articles/pbmc3k_tutorial.html) vignette. For convenience, we included the `pbmc` dataset raw counts (obtained from 10x genomics) in this package. We just need to load the packages and load the count matrix. 
...
Now we should be ready to do some cool visualization. We first convince ourselves that the Seurat ``FindMarkers`` function returns the same information as the manual differential expression test above!!! We are happy to see this!!! So basically, we can always use the ``FindMarkers`` function instead of doing things manually. 
...
We return to further up in the Seurat tutorial to see which of the TEST SET genes vary across the TRAINING SET PCs. 
