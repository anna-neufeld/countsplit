...
countsplit <- function(X, folds=2, epsilon=rep(1/folds, folds), overdisps = NULL) {
  ...
  if (length(overdisps) != NCOL(X)) {
    stop("You should provide one overdispersion parameter for every column of X.")
  }
  ...
  if (!identical(epsilon, rep(1/folds, folds))) {
    if (folds==2) {
      ...
      # This is the only remaining slow case, and would be an unusal choice of parameters.
