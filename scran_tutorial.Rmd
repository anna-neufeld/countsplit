...
We now extract the counts and perform count splitting. Note that these are all GENE BY CELL matrices- which is different than what we had before. 
...
All we need to make the ``SingleCellExperiment`` training object is the ``Xtrain`` counts matrix. But it might be nice to retain the column data? 
...
Now we are ready for our analysis! These steps were inspired by the `\textt{scran}``scran`` tutorial, available at \url{https://bioconductor.org/packages/release/bioc/vignettes/scran/inst/doc/scran.html}{https://bioconductor.org/packages/release/bioc/vignettes/scran/inst/doc/scran.html}.
...
It turns out that this function returned 11 clusters.
...
We now consider two ways to check differential expression. The first is Poisson GLMs for differentially expressed genes between 1 and 2. We don't need Xtest to be in a special object for this.For computational efficiency, we don't want to check all 21,000 genes. Let's check 500 randomly selected genes.
Recall that in Xtest, the genes are the ROWs not the columns. 
...
What if instead we try to use their own find markers function? It works, just need to figure out how to use. 
I think it does pairwise t-test between all of the things. 
...
This shows how different cluster 1 is from all of the other clusters. WRite a little paragaph about interpreting results. Ok! There really are important genes! Even without double dipping!!!
```{r}
